...
def prepare_mimic4_readmission_extract_from_raw(args):
    '''
    Extract mimic4 readmission data from raw data
    '''
    period_length = args.period_length
    # Include period_length in cache file path to avoid cache conflicts
    raw_data_path = args.mid_data_dump_path + f'/mimic4_readmission/mimic4_readmission_raw_period{period_length}.pkl'
    if os.path.exists(raw_data_path):
        print('Loading mimic4 readmission data from directory: ', raw_data_path)
...
def prepare_mimic4_readmission_for_smart(args, patient_all):
    """
    将 mimic4_readmission 数据适配为 SMART 模型输入格式
    ...
    """
    period_length = args.period_length
    smart_data_path = args.mid_data_dump_path + f'/mimic4_readmission/mimic4_readmission_smart_period{period_length}.pkl'
    if os.path.exists(smart_data_path):
        ...
        print('Adapting mimic4 readmission data for SMART model...')
        
        # SMART 模型期望的输入维度
        expected_input_dim = 44  # smart_input_dim for mimic4_readmission
...
def prepare_mimic4_readmission_train_val_test_split(args, patient_all):
    dump_path = os.path.join(args.mid_data_dump_path, f"mimic4_readmission/seed{args.seed}")
    ...
    pickle.dump(train_data, open(os.path.join(dump_path, 'mimic4_readmission_train.pkl'), 'wb'))

    val_data = {}
    for idx in patient_index[train_num:train_num + val_num]:
        for key in patient_all.keys():
            val_data.setdefault(key, []).append(patient_all[key][idx])
    pickle.dump(val_data, open(os.path.join(dump_path, 'mimic4_readmission_val.pkl'), 'wb'))

    test_data = {}
    for idx in patient_index[train_num + val_num:]:
        for key in patient_all.keys():
            test_data.setdefault(key, []).append(patient_all[key][idx])
    pickle.dump(test_data, open(os.path.join(dump_path, 'mimic4_readmission_test.pkl'), 'wb'))

    return train_data, val_data, test_data

def prepare(args):
    patient_all = prepare_mimic4_readmission_extract_from_raw(args)
    if args.method == "ehr_model_smart" or args.embedding_model_name == 'smart':
        patient_all = prepare_mimic4_readmission_for_smart(args, patient_all)
    train_data, val_data, test_data = prepare_mimic4_readmission_train_val_test_split(args, patient_all)
    return train_data, val_data, test_data
