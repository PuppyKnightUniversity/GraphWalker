...
def prepare_tjh_mortality_extract_from_raw(args):
    '''
    Extract tjh mortality data from raw data
    '''
    period_length = args.period_length
    # Include period_length in cache file path to avoid cache conflicts
    raw_data_path = args.mid_data_dump_path + f'/tjh_mortality/tjh_mortality_raw_period{period_length}.pkl'
    if os.path.exists(raw_data_path):
        print('Loading tjh mortality data from directory: ', raw_data_path)
        ...
        print(len(patient_all['X']))
        
        return patient_all

def prepare_tjh_mortality_wrap_prompt(args, patient_all):
    '''
        Wrap prompt for tjh mortality data
    '''
    raise NotImplementedError("tjh mortality data prompt wrapping is not implemented yet")

def prepare_tjh_mortality_for_smart(args, patient_all):
    """
    将 tjh_mortality 数据适配为 SMART 模型输入格式
    ...
    """
    period_length = args.period_length
    smart_data_path = args.mid_data_dump_path + f'/tjh_mortality/tjh_mortality_smart_period{period_length}.pkl'
    if os.path.exists(smart_data_path):
        ...
        print('Adapting tjh mortality data for SMART model...')
        
        # SMART 模型期望的输入维度
        expected_input_dim = 75  # smart_input_dim for tjh_mortality
...
def prepare_tjh_mortality_train_val_test_split(args, patient_all):
    '''
        Split the data into train, val, and test sets
        Test: 200 samples, Train: 140 samples, remaining: valid
    '''
    dump_path = os.path.join(args.mid_data_dump_path, f"tjh_mortality/seed{args.seed}")
    ...
    # 检查数据总量是否足够
    if N < test_num + train_num:
        raise ValueError(f"数据总量不足！需要至少 {test_num + train_num} 个样本，但只有 {N} 个")
    
    print(f"Total samples: {N}")
    
    # 先划分test集（200个）
    test_data = {}
    for idx in patient_index[:test_num]:
        for key in patient_all.keys():
            test_data.setdefault(key, []).append(patient_all[key][idx])
    pickle.dump(test_data, open(os.path.join(dump_path, 'tjh_mortality_test.pkl'), 'wb'))

    # 再划分train集（140个）
    train_data = {}
    for idx in patient_index[test_num:test_num + train_num]:
        for key in patient_all.keys():
            train_data.setdefault(key, []).append(patient_all[key][idx])
    pickle.dump(train_data, open(os.path.join(dump_path, 'tjh_mortality_train.pkl'), 'wb'))

    # 剩余的全部作为valid集
    val_data = {}
    for idx in patient_index[test_num + train_num:]:
        for key in patient_all.keys():
            val_data.setdefault(key, []).append(patient_all[key][idx])
    pickle.dump(val_data, open(os.path.join(dump_path, 'tjh_mortality_val.pkl'), 'wb'))
...
def prepare(args):
    patient_all = prepare_tjh_mortality_extract_from_raw(args)
    ...
    if args.method == "ehr_model_smart" or args.embedding_model_name == 'smart':
        patient_all = prepare_tjh_mortality_for_smart(args, patient_all)
        
    train_data, val_data, test_data = prepare_tjh_mortality_train_val_test_split(args, patient_all)
    return train_data, val_data, test_data
