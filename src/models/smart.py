...
class Encoder(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.embedder = MLPEmbedder(args.smart_d_model)
        self.query = nn.Parameter(torch.zeros(args.smart_input_dim, 1, args.smart_d_model))
        self.query.data.normal_(mean=0.0, std=0.02)
        self.position_enc = PositionalEncoding(args.smart_d_model, n_position=args.smart_max_len + 1)
        self.blocks = nn.ModuleList(
            [BasicBlock(dim=args.smart_d_model, num_heads=args.smart_n_heads, qkv_bias=False,
                        mlp_ratio=4., proj_drop=args.smart_dropout) for l in range(args.smart_e_layers)]
        )
...
class EmbeddingDecoder(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.mlp = Mlp(
            in_features=args.smart_d_model,
            hidden_features=int(args.smart_d_model * 4),
            act_layer=nn.GELU,
            drop=args.smart_dropout,
        )
        self.proj_out = nn.Linear(args.smart_d_model, args.smart_d_model)
...
class Classifier(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.mlp = MLPBlock(dim=args.smart_d_model, mlp_ratio=4, 
                            proj_drop=args.smart_dropout, act_layer=nn.GELU, norm_layer=nn.LayerNorm)
        self.out = nn.Linear(args.smart_d_model * args.smart_input_dim, args.smart_num_class)
    
    def get_logits_and_cls_token(self, h, **kwargs):
        '''
            Get the logits and cls token from the hidden states
        '''
        B, I, T, H = h.shape
        cls_token = h[:, :, 0]
